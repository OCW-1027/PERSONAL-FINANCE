...
const LANG = {
  ko: {
    // --- 공통 ---
    appTitle: "PFS", appSub: "Personal Finance System", appSub2: "개인사업 재무관리",
    ...
    installed: "설치했습니다",
    sync: "동기화",
    syncTitle: "클라우드 동기화 (Google)",
    login: "Google 로그인",
    logout: "로그아웃",
    loginNote: "로그인하면 PC·스마트폰에서 같은 데이터를 쓸 수 있습니다. 데이터는 본인 계정에만 저장되며 다른 사람은 접근할 수 없습니다.",
    notLoggedIn: "로그인하지 않음",
    upload: "업로드 (이 기기 → 클라우드)",
    download: "다운로드 (클라우드 → 이 기기)",
    uploaded: "업로드 완료",
    downloaded: "다운로드 완료",
    syncLast: "마지막 동기화: {n}일 전",
    syncNever: "아직 동기화하지 않았습니다",
    cloudInfo: "클라우드: {n}건 ({d})",
    cloudEmpty: "클라우드에 데이터 없음",
    confirmUpload: "클라우드 데이터({n}건)를 이 기기의 데이터({m}건)로 덮어씁니다. 계속할까요?",
    confirmDownload: "이 기기의 데이터({m}건)를 클라우드 데이터({n}건)로 덮어씁니다. 계속할까요?",
    syncErr: "동기화 실패. 네트워크·로그인 상태를 확인하세요.",
    autoSync: "저장 시 자동 업로드",
    aggregate: "종합과세",
  ...
  ja: {
    appTitle: "PFS", appSub: "Personal Finance System", appSub2: "個人事業 財務管理",
    ...
    sync: "同期",
    syncTitle: "クラウド同期 (Google)",
    login: "Googleでログイン",
    logout: "ログアウト",
    loginNote: "ログインするとPC・スマートフォンで同じデータを使えます。データはご自身のアカウントにのみ保存され、他の人はアクセスできません。",
    notLoggedIn: "未ログイン",
    upload: "アップロード (この端末 → クラウド)",
    download: "ダウンロード (クラウド → この端末)",
    uploaded: "アップロード完了",
    downloaded: "ダウンロード完了",
    syncLast: "最終同期: {n}日前",
    syncNever: "まだ同期していません",
    cloudInfo: "クラウド: {n}件 ({d})",
    cloudEmpty: "クラウドにデータなし",
    confirmUpload: "クラウドのデータ({n}件)をこの端末のデータ({m}件)で上書きします。続けますか？",
    confirmDownload: "この端末のデータ({m}件)をクラウドのデータ({n}件)で上書きします。続けますか？",
    syncErr: "同期に失敗しました。ネットワーク・ログイン状態を確認してください。",
    autoSync: "保存時に自動アップロード",
    aggregate: "総合課税",
